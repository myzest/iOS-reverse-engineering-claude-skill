...
**文档输出语言**: 所有生成的文档（README.md、分析报告、协议规范、hook 说明等）必须使用**中文**输出。
...
7. **Generate documentation** — Write README.md (in Chinese) with hook details, JSON config format, log file locations, and TrollFools instructions
...
| Hook log shows values captured (e.g., `[FlyVerifyContext setSdkVerifyAppKey:] = abc123`) but no structured JSON output file exists | Hooks log values but never call `saveJSONToTmp` or `writeToFile:` — logging alone is not enough | Every value-capture hook MUST save to file (Rule 8). Add `saveJSONToTmp(@"name.json", dict)` + Documents write to every `repl_` that captures a value |
| NLEAppConfig/C++-ivar class setter hooks all silent, but `valueForKey:` getter polling returns correct values | C++ ivar-backed ObjC properties — values written directly to C++ struct (`_cppModel`), setters never called, but ObjC getters bridge from C++ struct | Add getter polling (Rule 9): hook `-[ClassName init]` to capture instance, then `dispatch_after` + `valueForKey:` polling |
